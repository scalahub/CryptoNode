...
class PrvKey_P2PKH_UAHF(eccPrvKey: ECCPrvKey, mainNet: Boolean)
    extends PrvKey(eccPrvKey, mainNet) {
  ...
  def signTx(rawTx: Array[Byte], whichInputsAmts: Seq[(Int, BigInt)]) = { // which input indices to sign
    ...
    tx.outs.zipWithIndex.foreach {
      case (o, i) =>
        if (o.optAddress.isEmpty)
          throw new Exception(s"Empty output at index $i")
        val addr = o.optAddress.get
        getAddressType(getScriptPubKeyFromAddress(addr)) match {
          case Some(P2PKH_ADDRESS_TYPE) => // all ok
          case _ =>
            throw new Exception(
              s"Unsupported address type at index $i. Only P2PKH addresses are allowed."
            )
        }
    }
    whichInputsAmts.map {
      case (index, value) =>
        ...
        val sigBytes = eccPrvKey.signHash(
          hash
        ) :+ sigHash_UAHF_Byte // append a 0x40 to indicate SIGHASH_FORKID
        tx.ins(index)
          .setScriptSig(
            (sigBytes.size.toByte +: sigBytes) ++ (pubKey.bytes.size.toByte +: pubKey.bytes)
          )
    }
