...
object NetUtil {
  ...
  abstract class AbstractNetParser(bytes: Array[Byte]) extends AbstractParser(bytes) {
    ...
    def getNetAddrPayload(isVersionMsg: Boolean) =
      NetAddrPayload(if (isVersionMsg) 0 else getNext4UInt, getNext8UInt, getNextBytes(16).toArray, getNext2UInt)(isVersionMsg)
  }
