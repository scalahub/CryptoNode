...
class PeerGroup(listener: EventListener, config: PeerGroupConfig) extends Actor {
  ...
  val unknownTxHashes = MMap[TxID, (ActorRef, Time)]() // txid -> peer // txs seen by network but not by us.
  // Will store the txids received from INV messages
  ...
  def usingConnectedAsync[T](f: => T) =
    usingFailure(peers.size == 0)("Not connected")(f) match {
  ...
  def receive = {
    ...
    case blk: Blk => // received from REMOTE via a Peer
      ...
      if (blk.computeMerkleRoot == blk.header.merkleRoot) { // validate merkle root
        ...
        getBlockReq
          .get(blk.header.hash)
          .map {
            case (actorRef, time) => // for any pending "getBlock" requests,
    ...
    case (`getDataCmd`, inv: InvPayload) => // from peerActor (getData request received from remote side)
      inv.invVectors.collect {
        case InvVector(MSG_TX, char32) => // as of now, remote can only request for tx that we originate (i.e. in pushedTx)
          pushedTx.get(char32.rpcHash).map {
            case (tx, time) =>
    ...
    case inv: InvPayload => // from peerActor (inv from others, declaring new items -- tx or block)
      ...
      val invToSend = inv.invVectors.filter {
        case InvVector(MSG_BLOCK, char32) if seenBlkHashes.contains(char32.rpcHash) =>
          false // this is for a block which we have seen
        case InvVector(MSG_BLOCK, _) =>
          true // this is for a block which we have not seen. We need it
        case InvVector(MSG_TX, char32) if pushedTx.contains(char32.rpcHash) => // this is our tx
          pushedTx
            .get(char32.rpcHash)
            .map(seenTx += char32.rpcHash -> _) // if this is our tx, add to seen tx too (don't remove from pushedTx for now)
          false
        case InvVector(MSG_TX, char32) if seenTx.contains(char32.rpcHash) =>
          false
    ...
    case ("pushtx", tx: Tx) => // from Node (app has send us a "push" tx request
      // (via Node) and we need to send it to others)
      usingConnectedAsync {
