...
object Payloads {
  ...
  case class FilterLoadPayload(f: BloomFilter)
      extends Payload(
  ...
  case class AddrPayload(netAddrs: Seq[NetAddrPayload]) extends {
    ...
    def this(sAddresses: Array[InetSocketAddress]) =
      this(
        sAddresses.map(sAddress => new NetAddrPayload(sAddress.getAddress.getAddress, sAddress.getPort))
      )
  ...
  case class VersionPayload(
      ...
      services: UInt64,
      timeStamp: Int64,
      addr_recv: NetAddrPayload,
      addr_from: NetAddrPayload,
      nonce: UInt64,
      userAgent: String,
      startHeight: Int32,
      relay: Boolean
