...
case class ECCPrvKey(bigInt: BigInt, compressed: Boolean) {
  ...
  private[cryptonode] def signHash(hash: Array[Byte]) = {
  ...
  private[cryptonode] def sign(msg: String): String = (signHash(sha256Bytes2Bytes(msg.getBytes))).encodeHex

  @deprecated("Do not use in production. Only for testing!", "27 Dec 2017")
  private[cryptonode] def signMessageKeyRecovery(msg: String) = signHashKeyRecovery(dsha256(msg.getBytes)).encodeHex
}
