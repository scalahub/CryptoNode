...
object PrvKey {
  ...
  def getPrvKeyP2SH_P2WPKH(wif: String) = {
    ...
    if (!eccPrvKey.compressed)
      throw new Exception("P2SH_P2WPKH requires compressed private key")
...
abstract class PrvKey(val eccPrvKey: ECCPrvKey, val mainNet: Boolean) {
  val pubKey: PubKey
  def signTx(rawTx: Array[Byte], inputIndex: Int, amount: BigInt): Array[Byte] =
    signTx(rawTx, Seq((inputIndex, amount)))
  def signTx(rawTx: Array[Byte], whichInputs: Seq[(Int, BigInt)]): Array[Byte]

  @deprecated("Should not be exposed", "22 Dec 2017")
  def getWIF: String =
    getBase58FromBytes( // from BitcoinUtil
