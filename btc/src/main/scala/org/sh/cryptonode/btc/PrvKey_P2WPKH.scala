...
class PrvKey_P2WPKH(bigInt: BigInt, mainNet: Boolean)
    extends PrvKey(new ECCPrvKey(bigInt, true), mainNet) {

  lazy val pubKey = new PubKey_P2WPKH(eccPrvKey.eccPubKey.point, mainNet)
  ...
  def signTx(rawTx: Array[Byte], whichInputsAmts: Seq[(Int, BigInt)]) = { // which input indices to sign, with amount
    ...
    whichInputsAmts.map {
      case (i, value) =>
        val hash =
          tx.getHashSigned_P2SH_P2WPKH(i, value, doubleHashedPubKeyBytes)
        val sig = signHash(hash) ++ Array(
          0x01.toByte
        ) // append a 0x01 to indicate SIGHASH_ALL
