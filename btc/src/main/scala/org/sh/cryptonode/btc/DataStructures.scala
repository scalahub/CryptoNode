...
object DataStructures {
  ...
  case class Tx(
    ...
    def getHashSigned_P2PKH(whichInput: Int, inputAddress: String) = {
      ...
      dsha256(
        createNonSegWitTx(version, emptyIns, outs, lockTime) ++ sigHashAllBytes
    ...
    def getHashSigned_P2PKH_UAHF(whichInput: Int, inputAddress: String, value: BigInt) = { // which input indices to sign, with amount. Last param is pubKeyHashed via RIPEMD160(SHA256(_))
      ...
      val (scriptPubKey, isMainNetAddr) = getScriptPubKeyAndNetFromAddress(inputAddress)
      if (isMainNetAddr != isMainNet)
        throw new Exception(s"MainNet mismatch between $inputAddress (mainNet: $isMainNetAddr) and current setting (mainNet: $isMainNet)")
      ...
      val hashOuts = dsha256(
        outs.flatMap(out =>
          getFixedIntBytes(out.value, 8) ++ out.optScriptPubKey
            .map { scriptPubKey =>
    ...
    def getHashSigned_P2SH_P2PK(whichInput: Int, redeemScript: Seq[Byte]) = {
      ...
      dsha256(
        createNonSegWitTx(version, emptyIns, outs, lockTime) ++ sigHashAllBytes
      ) // this tx is appended with 01000000 (sigHashAll) ... // important !! in P2SH (nonSegwit), we need to sign old transaction serialized bytes ... and its double hash is signed
    }
    ...
    private def isInputSigned(index: Int, value: BigInt, optAddress: Option[Address]) = {
      if (wits(index).data.isEmpty) { // non SegWit Tx
        if (ins(index).optScriptSig.isEmpty) false
        else {
        ...
        if (optAddress.getOrElse(pubKey.address) != pubKey.address) false
        else eccPubKey.point.verify(getHashSigned_P2SH_P2WPKH(index, value, pubKey.doubleHashedPubKeyBytes), decodeDERSigBytes(sig.toArray))
      }
    }
  }
